---
title: "Actionable List Members API Requests"
domain: omnistudio
topic: actionable-list-members-api-requests
apiVersion: 67.0
release: summer-26-v67
docType: developer-guide
lastCollected: 2026-03-11T15:25:07.370Z
keywords: [Actionable, List, Members, API, Requests]
---

# Actionable List Members API Requests

# Actionable List Members API Requests

Here’s the list of the Actionable List Members API request bodies.

-   **[Actionable List Fields Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_actionable_list_fields_input.htm)**  
    Input representation for updating an actionable list record.
-   **[Actionable List Member Fields Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_actionable_list_member_fields_input.htm)**  
    Input representation for updating an actionable list member record.
-   **[Actionable List Update Member Count Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_actionable_list_update_member_count_input.htm)**  
    Actionable List Member Count input.
-   **[Actionable List Update Member Count Filter Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_actionable_list_update_member_count_filter_input.htm)**  
    Actionable List Member Count input for one filter.
-   **[Actionable List Update Member Count Filter List Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_actionable_list_update_member_count_filter_input_list.htm)**  
    Actionable List Member Count input for filters.
-   **[Assign Actionable List Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_assign_actionable_list_input.htm)**  
    Input representation for assigning an actionable list.
-   **[Assignee Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_assignee_input.htm)**  
    Input representation for updating the assignee details of an actionable list member.
-   **[Auto Assignee Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_auto_assignee_input.htm)**  
    Input representation for updating Auto Assignee.
-   **[Filter Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_filter.htm)**  
    Input representation for a filter query to fetch actionable list members.
-   **[OrderBy Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_order_by.htm)**  
    Input representation of the orderBy clause to sort actionable list members.
-   **[Search Actionable List Member Input](atlas.en-us.industries_reference.meta/industries_reference/connect_requests_search_actionable_list_member_input.htm)**  
    Input representation for searching for actionable list members.