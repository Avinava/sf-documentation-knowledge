...
| QuantityAllocated | TypedoublePropertiesCreate, Filter, Sort, UpdateDescriptionThe quantity of product items that are allocated but not yet shipped.. |
| QuantityAvailable | TypedoublePropertiesCreate, Filter, Sort, UpdateDescriptionThe quantity of product items that are currently available in inventory. |
| QuantityDamaged | TypedoublePropertiesCreate, Filter, Sort, UpdateDescriptionThe quantity of product items that are damaged. |
| QuantityInTransit | TypedoublePropertiesCreate, Filter, Sort, UpdateDescriptionThe quantity of product items that are currently in transit between locations. |
| QuantityOrdered | TypedoublePropertiesCreate, Filter, Sort, UpdateDescriptionThe quantity of product items that are ordered from suppliers but not yet received. |
| QtyStateRefreshDate | TypedatetimePropertiesCreate, Filter, Sort, UpdateDescriptionThe date and time when the quantity status of the product item was last updated. The date only changes when the quantity status changes, not when the on-hand quantity changes. |
