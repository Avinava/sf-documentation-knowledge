...
-   To filter records based on their categorization. See [WITH DATA CATEGORY](atlas.en-us.knowledge_dev.meta/knowledge_dev/sforce_api_calls_soql_select_with_datacategory.htm "You can search for Salesforce Knowledge articles and questions by their data category in a SOQL query. WITH DATA CATEGORY is an optional clause in a SELECT statement that’s used to filter records that are associated with one or more data categories and are visible to users.").
...
-   **[WITH DATA CATEGORY](atlas.en-us.knowledge_dev.meta/knowledge_dev/sforce_api_calls_soql_select_with_datacategory.htm)**
