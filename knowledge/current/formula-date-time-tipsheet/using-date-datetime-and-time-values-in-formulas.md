---
title: "Using Date, Date/Time, and Time Values in Formulas"
domain: formula-date-time-tipsheet
topic: using-date-datetime-and-time-values-in-formulas
apiVersion: 67.0
release: summer-26-v67
docType: api-reference
lastCollected: 2026-04-07T18:37:39.199Z
estimatedTokens: 2488
keywords: [Date, Time, Formulas, useful, managing, payment, deadlines, contract, ages, any, features, organization, dependent, TODAY, NOW, TIMENOW, Function, Converting, Text, Zones]
---

> Date formulas are useful for managing payment deadlines, contract ages, or any other
		features of your organization that are time or date dependent.
...
For example, if the difference between two Date/Time values is 5.52, that means the two values are separated by five days, 12 hours (0.5 of a day), and 28 minutes (0.02 of a day). You can also add numeric values to Dates and Date/Times. For example, the operation TODAY() + 3 returns three days after today’s date. For more information and examples of working with dates, see the list of [Sample Date Formulas](atlas.en-us.formula_date_time_tipsheet.meta/formula_date_time_tipsheet/formula_examples_dates.htm "Use the sample formulas in this topic to manipulate and perform calculations with date and time.").

Throughout the examples, the variables date and date/time are used in place of actual Date and Date/Time fields or values.

Keep in mind that complex date functions tend to compile to a larger size than text or number formula functions, so you might run into issues with formula compile size.
...
For details on how to convert between Date values and Date/Time values, see [Converting Between Date/Time and Date](#convert-date-dt).
...
If the inputted Y/M/D values result in an invalid date, the DATE() function returns an error, so error checking is an important part of working with Date values. You can read about methods for handling invalid dates in [Sample Date Formulas](atlas.en-us.formula_date_time_tipsheet.meta/formula_date_time_tipsheet/formula_examples_dates.htm "Use the sample formulas in this topic to manipulate and perform calculations with date and time.").
...
Let’s say a San Francisco user enters a value of 12:00 AM on August 2, 2013 in a custom Date/Time field called Date\_Time\_c. This value is stored as 2013–08–02 07:00:00Z, because the time difference in Pacific Daylight Time is GMT-7. At 12:00 p.m. PDT on August 1st, the user views the record and the following formula is run:
...
- Sample Date
					Formulas (atlas.en-us.formula_date_time_tipsheet.meta/formula_date_time_tipsheet/formula_examples_dates.htm)
- Sample Date Formulas (atlas.en-us.formula_date_time_tipsheet.meta/formula_date_time_tipsheet/formula_examples_dates.htm)
