---
title: "Public Sector Solutions Metadata API Types"
domain: psc-api
topic: public-sector-solutions-metadata-api-types
apiVersion: 67.0
release: summer-26-v67
docType: developer-guide
lastCollected: 2026-03-12T09:35:32.544Z
estimatedTokens: 333
keywords: [Sector, Solutions, Metadata, API, access, feature, settings, customize, user, Developer]
---

# Public Sector Solutions Metadata API Types

> Metadata API enables you to access some types and feature settings that you can
    customize in the user interface. For more information about Metadata API and to find a complete
    reference of existing metadata types, see Metadata API Developer Guide.

# Public Sector Solutions Metadata API Types

Metadata API enables you to access some types and feature settings that you can customize in the user interface. For more information about Metadata API and to find a complete reference of existing metadata types, see **[Metadata API Developer Guide](https://developer.salesforce.com/docs/atlas.en-us.260.0.api_meta.meta/api_meta/)**.

-   **[ApplicationRecordTypeConfig](atlas.en-us.psc_api.meta/psc_api/meta_applicationrecordtypeconfig.htm)**
    Configuration that maps object record types to an application. This object is available in API version 57.0 and later.
-   **[DocumentChecklistSettings](atlas.en-us.psc_api.meta/psc_api/meta_documentchecklistsettings.htm)**
    Represents an org’s DocumentChecklistItem settings.
-   **[Flow for Public Sector Solutions](atlas.en-us.psc_api.meta/psc_api/psc_flow_metadata_api.htm)**
    Represents the metadata associated with a flow. With Flow, you can create an application that navigates users through a series of screens to query and update records in the database. You can also execute logic and provide branching capability based on user input to build dynamic applications.
-   **[IndustriesSettings](atlas.en-us.psc_api.meta/psc_api/meta_psc_industriessettings.htm)**
    Represents settings for Public Sector Solutions Cloud.

## Related Topics

- ApplicationRecordTypeConfig (atlas.en-us.psc_api.meta/psc_api/meta_applicationrecordtypeconfig.htm)
- DocumentChecklistSettings (atlas.en-us.psc_api.meta/psc_api/meta_documentchecklistsettings.htm)
- Flow for Public Sector Solutions (atlas.en-us.psc_api.meta/psc_api/psc_flow_metadata_api.htm)
- IndustriesSettings (atlas.en-us.psc_api.meta/psc_api/meta_psc_industriessettings.htm)
