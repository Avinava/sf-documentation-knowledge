---
title: "RestApiEventLog"
domain: object-reference
topic: restapieventlog
apiVersion: 67.0
release: summer-26-v67
docType: api-reference
lastCollected: 2026-03-12T09:35:16.289Z
...
| ExceptionMessage | TypestringPropertiesFilter, Nillable, SortDescriptionThe exception message for a REST API request. An exception message gives details about errors in handling an API request, such as why an API request failed. For example: common.exception.ApiException: startDate cannot be more than 30 days ago. |
| FieldCount | TypedoublePropertiesFilter, Nillable, SortDescriptionThe number of fields or columns, where applicable. |
...
| Method | TypestringPropertiesFilter, Group, Nillable, SortDescriptionThe HTTP method of the request. For example: GET, POST, PUT, and so on. |
| ObjectName | TypestringPropertiesFilter, Group, Nillable, SortDescriptionThe name of the object accessed by the API request. For example: Account, Opportunity, Contact, and so on. |
...
| RowsProcessed | TypeintPropertiesFilter, Group, Nillable, SortDescriptionNumbers of rows that are processed by the REST API. |
