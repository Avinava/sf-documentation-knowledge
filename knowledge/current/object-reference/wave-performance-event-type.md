---
title: "Wave Performance Event Type"
domain: object-reference
topic: wave-performance-event-type
apiVersion: 67.0
...
| EPT | TypeNumberDescriptionThe experienced page time in milliseconds. |
| EVENT_TYPE | TypeStringDescriptionThe type of event. The value is always WavePerformance. |
| IS_INITIAL | TypeBooleanDescriptionIndicates whether the event is for the initial load of dashboard (true) or not (false). |
...
| NAME | TypeStringDescriptionThe asset title or query string. |
| ORGANIZATION_ID | TypeIdDescriptionThe 15-character ID of the organization.For example: 00D000000000123. |
| QUERY_ID | TypeStringDescriptionThis field is deprecated in API version 50.0. The number of queries can be determined using the Uri Event type logs, referenced here |
...
| TYPE | TypeStringDescriptionThe CRM Analytics object type. This field is scheduled for deprecation in v59.0. Use the new IS_INITIAL field to determine the log line type. |
...
- here (atlas.en-us.object_reference.meta/object_reference/sforce_api_objects_eventlogfile_uri.htm)
