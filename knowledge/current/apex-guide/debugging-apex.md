---
title: "Debugging Apex"
domain: apex-guide
topic: debugging-apex
apiVersion: 67.0
release: summer-26-v67
docType: developer-guide
lastCollected: 2026-03-12T05:14:33.112Z
estimatedTokens: 171
keywords: [Debugging, Apex, provides, debugging, support., debug, code, Developer, Console, logs.]
---

# Debugging Apex

> Apex provides debugging support. You can debug your Apex code using the Developer
        Console and debug logs.

# Debugging Apex

Apex provides debugging support. You can debug your Apex code using the Developer Console and debug logs.

To aid debugging in your code, Apex supports exception statements and custom exceptions. Also, Apex sends emails to developers for unhandled exceptions.

1.  [Debug Log](atlas.en-us.apexcode.meta/apexcode/apex_debugging_debug_log.htm)

2.  [Exceptions in Apex](atlas.en-us.apexcode.meta/apexcode/apex_exception_definition.htm)
    Exceptions note errors and other events that disrupt the normal flow of code execution. throw statements are used to generate exceptions, while try, catch, and finally statements are used to gracefully recover from exceptions.

## Related Topics

- Debug Log (atlas.en-us.apexcode.meta/apexcode/apex_debugging_debug_log.htm)
- Exceptions in Apex (atlas.en-us.apexcode.meta/apexcode/apex_exception_definition.htm)
