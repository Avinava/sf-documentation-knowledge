...
lastCollected: 2026-05-10T00:40:44.112Z
estimatedTokens: 5076
...
These field types extend [primitive data types](atlas.en-us.api.meta/api/primitive_data_types.htm). Many of these field types follow common data typing conventions that are made explicit in their metadata. However, certain field types have unique characteristics that you must understand before using them in your client application.
...
![Case-insenstive applications give incorrect results when comparing 15-character IDs](/docs/resources/img/en-us/260.0?doc_id=api%2Fimages%2Fcase_of_ids.1.png&folder=api)
...
![Salesforce gives an error if you change the case of an 18-character ID](/docs/resources/img/en-us/260.0?doc_id=api%2Fimages%2Fcase_of_ids.2.png&folder=api)
...
The restriction is (total number of entity records) \* (total number of records in the entity's JunctionIdList field) must equal less than 500. For example, you query on the EventWhoIds JunctionIdList field for a list of events. There are 101 events and for each event, there are 5 records in the EventWhoIds JunctionIdList. Therefore, the Salesforce Object Query Language query would be querying for 505 records in total, which is over the 500 limit, and you get an exception.

To avoid exceeding the record limit, reformulate the query so it includes fewer than 500 junction IDs or query a parent/child relationship instead of junction IDs. For example, this Salesforce Object Query Language query includes CcIds, which is a JunctionIdList field.
...
- primitive data types (atlas.en-us.api.meta/api/primitive_data_types.htm)
