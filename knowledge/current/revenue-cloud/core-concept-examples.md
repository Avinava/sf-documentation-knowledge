...
For an explanation of the constraint model structure in these examples, see [Model Structure](atlas.en-us.revenue_lifecycle_management_dev_guide.meta/revenue_lifecycle_management_dev_guide/cml_appendix_model_structure.htm "The tables on the following pages show the structure for the constraint model in Core Concept Examples.").
...
Rules depending on these external variables (such as the ShippingCountry message) only re-evaluate when a Line Item change occurs (For example, updating requiredKW), not solely when the header field changes.
...
- Model
          Structure (atlas.en-us.revenue_lifecycle_management_dev_guide.meta/revenue_lifecycle_management_dev_guide/cml_appendix_model_structure.htm)
