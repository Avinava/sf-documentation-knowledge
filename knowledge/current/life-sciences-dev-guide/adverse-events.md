---
title: "Adverse Events"
domain: life-sciences-dev-guide
topic: adverse-events
apiVersion: 67.0
release: summer-26-v67
docType: help-article
lastCollected: 2026-04-07T18:37:57.507Z
estimatedTokens: 246
keywords: [Adverse, Events, model, helps, managing, documenting, any, unfavorable, unintended, signs, symptoms, diseases, occur, participants, research, study, include, wide, range, physical, psychological, effects, varies, mild, discomfort, serious, health, complications]
---

> Adverse Events data model helps you in managing and documenting any unfavorable or
    unintended signs, symptoms, or diseases that occur in participants during the research study.
    These adverse events can include a wide range of physical or psychological effects that varies
    from mild discomfort to serious health complications.

# Adverse Events

Adverse Events data model helps you in managing and documenting any unfavorable or unintended signs, symptoms, or diseases that occur in participants during the research study. These adverse events can include a wide range of physical or psychological effects that varies from mild discomfort to serious health complications.

| Available in: Lightning ExperienceAvailable in: Enterprise and Unlimited Editions with Life Sciences Cloud or Health Cloud |
| --- |


The adverse events objects are available to users with Health Cloud Starter or Health Cloud license. Available in API version 61.0 and later.

![Adverse events entity verification diagram](/docs/resources/img/en-us/260.0?doc_id=dev_guides%2Flife_sciences%2Fimages%2Flsc_adverse_events_erd.png&folder=life_sciences_dev_guide)

For more details and a larger image, visit the [Data Model Gallery](https://developer.salesforce.com/docs/platform/data-models/guide/adverse-events.html "HTML (New Window)").
