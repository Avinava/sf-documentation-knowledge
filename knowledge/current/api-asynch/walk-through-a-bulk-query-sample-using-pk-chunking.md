---
title: "Walk Through a Bulk Query Sample Using PK Chunking"
domain: api-asynch
topic: walk-through-a-bulk-query-sample-using-pk-chunking
apiVersion: 67.0
release: summer-26-v67
docType: developer-guide
lastCollected: 2026-03-12T09:33:59.164Z
estimatedTokens: 1719
keywords: [Walk, Bulk, Query, Sample, Chunking, code, uses, cURL, perform, enabled, several, account, records, Job, Add]
---

# Walk Through a Bulk Query Sample Using PK Chunking

> This code sample uses cURL to perform a bulk query with PK chunking enabled on several
    account records.
...
2.  Using a command-line window, execute this cURL command to create a job with PK chunking enabled.
    ...
    curl -H "X-SFDC-Session: **sessionId**" -H "Content-Type: application/xml; charset=UTF-8" -H "Sforce-Enable-PKChunking: true" -d @create-job.xml https://**instance**.salesforce.com/services/async/66.0/job
...
curl -H "X-SFDC-Session: **sessionId**" https://**instance**.salesforce.com/services/async/66.0/job/**jobId**/batch
