---
title: "Platform Event Schema by Schema ID"
domain: rest-api
topic: platform-event-schema-by-schema-id
apiVersion: 67.0
release: summer-26-v67
docType: api-reference
lastCollected: 2026-05-10T00:39:57.533Z
...
This URI gets the schema of a platform event whose schema ID is 5E5OtZj5\_Gm6Vax9XMXH9A. This schema ID is a sample ID. Replace it with a valid schema ID for your event.
...
In API version 43.0 and later, the response format is the JSON representation of the event schema by default. The returned response looks like the following in API version 66.0.
...
The returned response for the compact format looks like the following in API version 66.0.
...
The compact schema doesn’t include the replayId or channel fields because these fields aren’t necessary for deserializing the compact event received.

Custom formula fields contained within Change Data Capture events include a doc tag in the format "doc" : "Formula:Text:00NSB00004I3D4L",".
...
This format is what the API returns in API version 43.0 and later when appending the ?payloadFormat=COMPACT parameter.
...
When you change the definition of a platform event, the schema ID for this platform event changes.

If you don’t have the schema ID, you can get the schema by supplying the platform event name. Make a GET request to /services/data/vXX.X/sobjects/eventName/eventSchema. See [Platform Event Schema by Event Name](atlas.en-us.api_rest.meta/api_rest/resources_sobject_eventschema.htm "Gets the definition of a platform event in JSON format for an event name. This resource is available in REST API version 40.0 and later.").

## Apache Avro Format

The fields in the returned response adhere to the open-source Apache Avro specification for the record complex type (see [Avro Records](https://avro.apache.org/docs/1.8.1/spec.html#schema_record "HTML (New Window)") in the Apache Avro specification). Note the following:
