---
title: "Bucket Date Setup Input"
domain: salesforce-recipes-api
topic: bucket-date-setup-input
apiVersion: 67.0
release: summer-26-v67
docType: help-article
lastCollected: 2026-04-07T18:39:18.110Z
estimatedTokens: 148
keywords: [Bucket, Date, Setup, node, recipe]
---

> The date setup for a bucket node in a recipe.

# Bucket Date Setup Input

The date setup for a bucket node in a recipe.

## Properties

Inherits properties from [Bucket Setup Input](atlas.en-us.salesforce_recipes_api.meta/salesforce_recipes_api/sforce_recipes_api_requests_bucket_setup_input.htm "The base field setup for a bucket node in a recipe.").

| Property Name | Type | Description | Required or Optional | Available Version |
| --- | --- | --- | --- | --- |
| buckets | Bucket​Date​Bucket​Input[] | The list of buckets. | Required | 51.0 |
| source​Field | Bucket​Date​Source​Field​Input | The source field. | Required | 51.0 |

## Related Topics

- Bucket Setup Input (atlas.en-us.salesforce_recipes_api.meta/salesforce_recipes_api/sforce_recipes_api_requests_bucket_setup_input.htm)
- Bucket​Date​Bucket​Input (atlas.en-us.salesforce_recipes_api.meta/salesforce_recipes_api/sforce_recipes_api_requests_bucket_date_bucket_input.htm)
- Bucket​Date​Source​Field​Input (atlas.en-us.salesforce_recipes_api.meta/salesforce_recipes_api/sforce_recipes_api_requests_bucket_date_source_field_input.htm)
