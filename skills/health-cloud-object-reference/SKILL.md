...
1. **Use the `view_file` tool** to read the index file at `knowledge/current/health-cloud-object-reference/_index.md`.
2. **Identify relevant topics** from the Markdown table in the index.
3. **Read the specific topic files** in `knowledge/current/health-cloud-object-reference/` (e.g. `knowledge/current/health-cloud-object-reference/topic-name.md`) to get detailed implementation steps, JSON formats, or API references.
