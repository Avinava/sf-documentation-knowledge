...
import { getDomainById, getDomainsByPriority, getCollectableDomains } from '../config/domains.js';
    ...
    .action(async (options) => {
        ...
        if (options.domain) {
            ...
            if (!domain) {
                ...
                console.error('Available domains:', getCollectableDomains().map((d) => d.id).join(', '));
                process.exit(1);
            }
        ...
        for (const r of results) {
            ...
            console.log(`${status} ${r.domain}: ${r.filesCollected} files (${r.durationMs}ms)`);
