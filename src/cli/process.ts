...
program
    ...
    .option('-o, --output <dir>', 'Output directory for processed data', 'data/processed')
    .action(async (options) => {
        ...
        for (const domain of domains) {
            ...
            if (!(await fs.pathExists(rawDir))) {
                log.warn({ domain: domain.id, rawDir }, 'No raw data found — run collect first');
                continue;
            }
            ...
            log.info({ domain: domain.id, fileCount: files.length }, 'Processing raw files');
            ...
            await fs.writeJson(path.join(outputDir, 'manifest.json'), allDocs, { spaces: 2 });
            log.info({ domain: domain.id, documentCount: allDocs.length }, 'Processing complete');
        }
    });
