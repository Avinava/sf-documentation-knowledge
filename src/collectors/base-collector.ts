...
export abstract class BaseCollector {
    ...
    constructor(domain: DomainConfig, baseOutputDir = 'data/raw') {
        ...
        this.rateLimitMs = parseInt(process.env.COLLECTION_RATE_LIMIT_MS || '500', 10);
    ...
    protected async fetch<T = unknown>(url: string): Promise<T> {
        ...
        for (let attempt = 1; attempt <= this.retryCount; attempt++) {
            try {
                ...
                if (attempt > 1) {
                    await this.sleep(this.rateLimitMs * attempt); // exponential backoff
                } else {
                ...
                this.log.warn(
                    { url, attempt, maxAttempts: this.retryCount, status: axiosErr.response?.status },
