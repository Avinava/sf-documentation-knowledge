...
export interface DocumentMetadata {
    ...
    docType: 'developer-guide' | 'api-reference' | 'release-note' | 'help-article';
    keywords: string[];
}
...
export function tagDocument(
    doc: { title: string; shortDescription: string; content: string; sourceId: string; sections: { heading: string }[] },
    domain: DomainConfig,
): TaggedDocument {
    ...
    const keywords = extractKeywords(doc.title, doc.shortDescription, doc.sections.map((s) => s.heading));
...
function extractKeywords(title: string, description: string, headings: string[]): string[] {
...
function detectDocType(
    ...
    if (lower.includes('api reference') || lower.includes('method') || lower.includes('parameters')) {
        return 'api-reference';
    }
    if (lower.includes('help') || lower.includes('how to') || lower.includes('setup')) {
        return 'help-article';
    }
...
const STOP_WORDS = new Set([
    ...
    'one', 'our', 'out', 'use', 'this', 'that', 'with', 'from', 'have', 'been', 'will',
    'each', 'make', 'like', 'when', 'what', 'your', 'some', 'them', 'than', 'into',
    'also', 'more', 'about', 'using', 'other',
]);
